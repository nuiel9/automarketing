...
```python
    ...
    # we give up and fail the item. Below the render Cloud Run job's
    # --task-timeout=15m (900s) so poll()'s own timeout error fires before
    # the task is killed.
