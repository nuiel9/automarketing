...
from app.video.ad_copy import BannedCopyError, write_ad_copy  # noqa: F401
...
`BannedCopyError` is imported but not referenced in this module: the existing
`except Exception` in `render_item` already catches it. Keeping the import
documents that this is deliberate — if a future change starts special-casing
it, the name is already here. (Drop the `noqa` if the linter is fine with it.)
